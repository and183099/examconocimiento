helados = []  # Lista para almacenar los helados  Liliana Andrea Gonzalez Marin
...
while True:
    ...
    if opcion == "1":  # Agregar un helado
        ...
        if precio.replace('.', '', 1).isdigit(): # Permitir decimales
            precio = float(precio)
        ...
        if len(helados) == 0:
            ...
            for helado in helados:
                print("ID: {helado['iD']}, Nombre: {helado['nombre']}, Descripción: {helado['descripcion']}, Precio: ${helado['precio']}")  # Error en claves del diccionario
